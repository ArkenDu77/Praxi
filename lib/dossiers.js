...
function upsertPatientFromIntake(userId, intakeId, champs = {}) {
  ...
  if (!idIntake) return { error: 'intake_id requis.' };
  ...
  const db  = readDossiers();
  const idx = db.patients.findIndex(p => p.userId === userId && p.intakeId === idIntake);
  const maintenant = new Date().toISOString();

  if (idx !== -1) {
    db.patients[idx] = { ...db.patients[idx], ...base, updatedAt: maintenant };
    writeDossiers(db);
