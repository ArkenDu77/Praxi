...
const { recordClinicalChange } = require('./previsit');
...
function updatePatient(userId, patientId, body) {
  ...
  db.patients[idx] = recordClinicalChange(existing, { ...existing, ...base, updatedAt }, { source: 'fiche médecin', at: updatedAt });
  writeDossiers(db);
...
function upsertPatientFromIntake(userId, intakeId, champs = {}) {
  ...
  if (existant) {
    ...
    db.patients[idx] = recordClinicalChange(existant, { ...fusion, updatedAt: maintenant }, { source: 'pré-consultation', at: maintenant });
    writeDossiers(db);
