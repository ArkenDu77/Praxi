...
/**
 * Emails à accès illimité : les comptes fondateurs, PLUS ceux
 * qu'ARKIBA_ADMIN_EMAILS ajoute (séparés par des virgules).
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  LA VARIABLE AJOUTE. ELLE NE REMPLACE PAS.
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * Elle remplaçait la liste. Un déploiement qui posait ARKIBA_ADMIN_EMAILS
 * pour une raison quelconque — ouvrir l'accès à un testeur, par exemple —
 * retirait donc l'accès illimité aux comptes fondateurs, en silence, et
 * `normaliserCompte()` allait jusqu'à baisser leur drapeau en base au
 * passage suivant. Personne n'aurait vu le rapport entre les deux.
 *
 * Un réglage d'exploitation ne doit pas pouvoir retirer un droit qui ne lui
 * appartient pas. Il ouvre, il ne ferme pas : pour retirer un compte
 * fondateur, il faut modifier ce fichier, ce qui laisse une trace dans
 * l'historique.
 */
function adminEmails() {
  const ajouts = (process.env.ARKIBA_ADMIN_EMAILS || '')
    .split(',')
    .map(e => e.trim().toLowerCase())
    .filter(Boolean);
  return new Set([...ADMIN_EMAILS_PAR_DEFAUT.map(e => e.toLowerCase()), ...ajouts]);
}
...
function normaliserCompte(user, maintenant = Date.now()) {
  ...
  /**
   * CABINET DE RATTACHEMENT — retrofit des comptes anterieurs.
   *
   * Les comptes crees avant l'existence des organisations n'en portent pas.
   * Sans ce rattrapage ils resteraient tous dans le cabinet de laboratoire, et
   * deux medecins reels partageraient le meme espace de donnees.
   *
   * La valeur derive de l'identifiant du compte : reproductible, lisible dans
   * un journal, et sans collision possible. `normaliserCompte` est appele a
   * chaque lecture de compte et persiste ce qu'il change — le rattrapage se
   * fait donc a la premiere connexion, sans migration a lancer.
   */
  if (!user.organizationId && user.id !== undefined) {
    user.organizationId = `org-${user.id}`;
    modifie = true;
  }
